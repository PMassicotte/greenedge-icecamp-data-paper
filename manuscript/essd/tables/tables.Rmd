...
df <- gs_title("table2") %>%
  ...
  select(Year = year, Parameter = parameter_for_table_2, `Sampling method` = sampling, `Principal investigators` = p_i, Processed = r_script) %>% 
  mutate(Processed = case_when(
    Processed == "NODATA" ~ "Data not available yet",
    TRUE ~ "Available"
  ))

kable(df, "latex", longtable = TRUE, booktabs = TRUE, caption = "Parameters measured during the Green Edge ice camp surveys. Parameters are ordered by alphabetical order and sampling year.") %>%
  kable_styling(latex_options = c("repeat_header"), font_size = 10) %>%
